...
- 🚨 **`airfs status` tells you which config the daemon actually loaded.** A daemon started with `--config` holds that file for its whole life, so the file you are editing and the file being served can drift apart — and every symptom of that looks like `airfs` ignoring your edit. Now it says so, on its own line. It also reads the kernel rather than taking the daemon's word for it, so a mountpoint that went stale or vanished from under a running daemon is named and exits `2`, however healthy the daemon believes it is.
- 🔗 **`airfs link --claude --opencode` — one place to write a project's own skills.** Your project owns skills, and every tool insists on looking somewhere else for them. Run this in the project and each named tool gets a relative, committable symlink to `.ai/skills/`, with whatever the tools already held moved in and every move named in the report. Nothing reconciles the symlinks — that is the point, since your teammates without `airfs` still need them to work. [Linking tools](https://sylvanld.github.io/airfs/user-guide/linking-tools/).
